...
git clone https://github.com/IamCaaarll/My_Portfolio.git MyPortfolio
...
- Navigate to `package.json` and enter your domain name instead of `https://myportfolio.js.org/` in `homepage` variable. For example, if you want your site to be `https://<your-username>.github.io/myPortfolio`, add the same to the homepage section of `package.json`.

- In short you can also add `/myPortfolio` to `package.json` as both are exactly same. Upon doing so, you tell `create-react-app` to add the path assets accordingly.
